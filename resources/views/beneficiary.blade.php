          ...
          <table id="datatable" class="table table-striped table-borderless text-center" >
            ...
            <tbody  >
              <tr style="text-align: center" >
                <td style="font-size: 1em" class="tid" > {{ $beneficiary->beneficiary->id }} </td>
                <td class="font-weight-bold tben_name" style="font-size:  1em">{{ $beneficiary->name }} </td>
                 <td class="font-weight-bold tben_card" style="font-size:  1em"> {{ $beneficiary->beneficiary->ben_cardnum}}</td>
                 <td class="font-weight-bold tben_phone" style="font-size:  1em"> {{ $beneficiary->beneficiary->ben_phone}}</td>
                 <td class="font-weight-bold tneigh_name" style="font-size:  1em"> {{ $beneficiary->beneficiary->neighborhood->neigh_name}}</td> 
                 @if( $beneficiary->beneficiary->allowed_in_month == 8)
                 <td class="font-weight-bold tallowed_in_month" style="font-size:  1em ; color:red"> حـرجـة جـداً</td> 
                @endif
               
                @if ( $beneficiary->beneficiary->allowed_in_month  == 6)
                <td class="font-weight-bold tallowed_in_month" style="font-size:  1em ; color:rgb(255, 170, 13)"> حـرجـة </td> 
                @endif

                @if ( $beneficiary->beneficiary->allowed_in_month  == 4)
                <td class="font-weight-bold tallowed_in_month" style="font-size:  1em ; color:rgb(53, 194, 11)"> مـحـتـاج</td> 
                @endif

                @if ( $beneficiary->beneficiary->allowed_in_month  == 1)
                <td class="font-weight-bold tallowed_in_month" style="font-size:  1em ; color:rgb(150, 167, 150)"> غير مؤكد</td> 
                @endif
               
                @if ( $beneficiary->beneficiary->allowed_in_month ==0)
                <td class="font-weight-bold tallowed_in_month" style="font-size:  1em ; color:rgb(150, 167, 150)"> غير محتاج </td> 
                ...
                <td> 
                   ...
                   <a href={{ "deleteBen/".$beneficiary->id }}>  <button type="button" class="btn btn-danger" style="font-weight: bolder; font-size: 1em">حـذف</button></a> 
                   <button type="button" class="btn editbtn" wire:click="editben{{ $beneficiary->id  }}"  data-toggle="modal"  data-target="#editModal" style="font-weight: bolder; font-size: 1em" value="{{ $beneficiary->id }}" >تـعـديـل</button> 


                </td>
                <td class=" tuser_id" style="display: none" >{{ $beneficiary->id }} </td>
          ...
          <div class="modal-body">
              ...
              <div class="form-group">
                <form action="{{ url('editben') }}" method="POST" id="editForm">
                  @csrf
                  <input type="hidden" name="id" :value="$beneficiary->id" id="id">
                  ...
                  <select class="form-control" name="neigh_id" >
                    ...
                    <input type="radio" value="8" name="allowed_in_month" id="حرجـة جـداً" style="margin-right: 15%; color: red" checked>
...
<script>
  ...
  $(document).on('click','.editbtn', function (e) {
   ...
   $('#allowed_in_month').val(_this.find('.tallowed_in_month'));
