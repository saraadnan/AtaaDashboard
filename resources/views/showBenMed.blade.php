            ...
            <tbody  >
              <tr style="text-align: center" >
                 ...
                 <td class="font-weight-bold tneigh_name" style="font-size:  1em"> {{ $ben->neighborhood->neigh_name}}</td> 
                    ...
                    @if( $ben->allowed_in_month == 8)
                    <td class="font-weight-bold tben_degree" style="font-size:  1em ; color:red"> حـرجـة جـداً</td> 
                   @endif
                  
                   @if ( $ben->allowed_in_month  == 6)
                   <td class="font-weight-bold tben_degree" style="font-size:  1em ; color:rgb(255, 170, 13)"> حـرجـة </td> 
                   @endif
   
                   @if ( $ben->allowed_in_month  == 4)
                   <td class="font-weight-bold tben_degree" style="font-size:  1em ; color:rgb(53, 194, 11)"> مـحـتـاج</td> 
                   @endif
   
                   @if ( $ben->allowed_in_month  == 1)
                   <td class="font-weight-bold tben_degree" style="font-size:  1em ; color:rgb(150, 167, 150)"> غير مؤكد</td> 
                   @endif
                  
                   @if ( $ben->allowed_in_month ==0)
                   <td class="font-weight-bold tben_degree" style="font-size:  1em ; color:rgb(150, 167, 150)"> غير محتاج  </td> 
                   {{-- @else
                   <td class="font-weight-bold tben_degree" style="font-size:  1em ; color:#181f39 ; text-align: center"> {{$ben->allowed_in_month  }} </td>  --}}
                   @endif

                {{-- <td> 
                 
                   <a href={{ "deleteBen/".$ben['ben_id'] }}>  <button type="button" class="btn btn-danger" style="font-weight: bolder; font-size: 1em">حـذف</button></a> 
                   <button type="button" class="btn editbtn" wire:click="editben{{ $ben->ben_id  }}"  data-toggle="modal"  data-target="#editModal" style="font-weight: bolder; font-size: 1em" value="{{ $ben->ben_id }}" >تـعـديـل</button> 
              ...
              <div class="form-group">
                <form action="{{ url('editben') }}" method="POST" id="editForm">
                  ...
                  <select class="form-control" name="neigh_id" >
                    ...
                    <input type="radio" value="8" name="ben_degree_need" id="حرجـة جـداً" style="margin-right: 15%; color: red" checked>
                    <label for="ben_degree_need" style="font-size: 1.2em ; padding-bottom: 1%; color: red"  > حرجـة جـداً</label> 
                   <br>
                    <input type="radio" value="6" name="ben_degree_need" id="حـرجـة" style="margin-right: 15%; color: rgb(255, 170, 13)">
                    <label for="ben_degree_need" style="font-size: 1.2em ; padding-bottom: 1% ; color: rgb(255, 170, 13)"  > حـرجـة</label> 
                    <br>
    
                    <input type="radio" name="ben_degree_need" value="4" style="margin-right: 15%;" >
