         ...
         <button type="button" class="btn " id ="button_add_donors" >اضـافة صنـف</button>
           ...
           <div class="table-responsive">
             <table id="datatable" class="table table-striped table-borderless">
               ...
               <tbody >
                 <tr>
                   <td style="font-size: 1em" class="tcat_id"> {{ $category->id }}</td>
                   <td class="font-weight-bold tcat_name" style="font-size:  1em">{{ $category->cat_name }}</td>
                   <td>
                   
                      <a href={{ "deleteCategory/".$category['id'] }}>  <button type="button" class="btn btn-danger" style="font-weight: bolder; font-size: 1em">حـذف</button></a> 
                      <button type="button" class="btn editbtn" wire:click="editgov{{ $category->id  }}"  data-toggle="modal"  data-target="#editModal" style="font-weight: bolder; font-size: 1em" value="{{ $category->id }}" >تـعـديـل</button> 
