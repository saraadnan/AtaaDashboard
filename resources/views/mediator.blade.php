          ...
          <table id="datatable" class="table table-striped table-borderless text-center">
            <thead>
              <tr >
                ...
                {{-- <th style="font-size:  1em"> عدد المستفيدين التابعين له </th> --}}
            ...
            <tbody >
              <tr>
                <td style="font-size: 1em" class="tmed_id"> {{ $mediator->mediator->med_id }}</td>
                {{-- <td class="font-weight-bold tmed_name" style="font-size:  1em">{{ $mediator->med_name }}</td> --}}
                <td class="font-weight-bold tmed_name" style="font-size:  1em">{{ $mediator->name }}</td>
                <td class="font-weight-bold tmed_card" style="font-size:  1em"> {{ $mediator->mediator->med_cardnum}}</td>
                 <td class="font-weight-bold tmed_phone" style="font-size:  1em"> {{ $mediator->mediator->med_phone}}</td> 

                 <td class="font-weight-bold tneigh_id" style="font-size:  1em"> {{ $mediator->mediator->neighborhood->neigh_name}}</td> 
                 ...
                 <td>
                   ...
                   <button type="button" class="btn editbtn" wire:click="editmed{{ $mediator->id  }}"  data-toggle="modal"  data-target="#editModal" style="font-weight: bolder; font-size: 1em" value="{{ $mediator->med_id }}" >تـعـديـل</button> 


                </td>
                <td class=" tuser_id"  style="display:none;" >{{ $mediator->id }} </td>
          ...
          <div class="modal-body">
              ...
              <div class="form-group">
                <form action="{{ url('editmed') }}" method="POST" id="editForm">
                  @csrf
                  <input type="hidden" name="med_id" :value="$mediator->mediator->id" id="med_id">
                  <input type="hidden" name="user_id" :value="$beneficiary->id" id="user_id">
